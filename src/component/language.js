...
UBS基金会DAPP，旨在为UBS系列生态打造去中心化的基金会运营平台。通过“权重+治理+分红”三大核心功能，让符合社区要求的UBS忠实用户都可以参与，通过社区推荐并满足权重要求者可以获得UBS基金会理事资格。UBS基金会理事享有对UBS系列生态的最高治理权和分红权，也将成为运营UBS系列生态的核心力量。
UBS系列生态包括：银行、交易所、矿池、彩票、借贷、NFT、区块浏览器、基金会等以及未来将可能出现的更多生态。

主要功能介绍
一 【权重】
1.1 权重分为SERO权重及UBSC权重，两个分项权重各占综合权重比例的50%，两项比例之和即为该账户的权重比例，权重比例是参与“治理”及“分红”的唯一指标。
1.2 激活理事资格：①在当前基金会（或原UBS生态）界面接入原点分享码的账户，投资/复投累积≥20万SERO（通过原UBS生态投资也可计算累积投资量并计入权重比例，但显示的权重比例为预测值，须再一次在基金会投资后，权重比例数据才会与链上数据同步，显示出当前账户真实的权重比例）；②通过社区邮箱推荐，将该账户主收款码的完整地址发送给开发者邮箱。同时满足以上两点，可激活基金会理事账户资格，激活后可显示权重比例。
1.3 注销理事资格：①激活UBS基金会理事资格后，当账户实际币量第一次低于20万SERO时，合约将暂时取消该账户的UBS基金会理事资格且失去权重比例，最多有一次机会可以再次投资使得账户实际币量≥20万SERO，可再次激活该账户的基金会理事资格。如果该账户的实际币量第二次低于20万SERO，该账户将被自动永久注销基金会理事资格；②通过社区邮箱反馈，可以注销指定收款地址对应的账户的理事资格。满足以上任意一点，账户都会被注销基金会理事资格，同时失去理事的所有权益。
1.4 权重比例：当前账户的实际投资币量/所有理事账户的总实际投资币量=当前账户的权重比例
1.5 理事等级：按权重币量分为三个等级，不同等级对应不同的权益（权重加速）。铜牌理事，即 20万SERO≤权重币量＜100万SERO，权重加速为0%；银牌理事，即100万SERO≤权重币量＜300万SERO，权重加速为5%；金牌理事，即权重币量≥300万SERO，权重加速为10%。
1.5 UBSC权重规则，将类同上述SERO权重规则，待UBSC正式发行前定义，目前仅以SERO权重比例计为理事账户的权重比例。

二 【治理】
2.1 关于对UBS系列生态的运营方案、使用操作、新增/修改功能等建议，都可以在“治理”功能中进行发起提案。
2.2 发起提案后，由所有基金会理事对提案进行投票表决（同意/反对），投票比例以权重比例进行计算。当投票“同意”比例超过50%，该提案将被执行。当投票“反对”比例超过50%，该提案将自动作废。
2.3 提案执行方式：①所有需要开发者执行的提案内容，在编辑提案时勾选“发送给开发者”选项，在该提案经过投票通过后，将被自动转发给开发者，比如需要转出分红池中xxx SERO/UBSC到xxx地址，比如需要注销某位理事账户（需要填对应收款地址）等功能，比如需要修改/新增某项功能等，将由开发者收到提案后执行。②所有运营方案，只需计算该方案通过的投票比例，无需发送给开发者则无需勾选“发送给开发者”选项，由社区组织投票并计算结果即可。

三 【分红】
3.1 基金会分红池包括：SERO分红池和UBSC分红池。
3.2 分红池的来源：①UBS银行合约池每天流入量的2%将自动注入进基金会SERO分红池中；②UBSC上线后，每天UBSC发行量的8%将自动注入进基金会的UBSC分红池中；③任何玩家都可以捐献SERO和UBSC给分红池。
3.2 分红池的支出：①按SERO和UBSC分红池的一定比例，每月自动分配一次分红基金给基金会理事账户；②按SERO分红池的一定比例，定期自动回购及燃烧UBSC。③基金会的运营费用，可通过提案投票进行支出。

上述的所有合约规则皆为拟定版本，目前尚未完全实现，所有技术功能的实现需要配合SERO 2.0技术才可以完成。在正式版的开发过程中可能会对上述规则有适度的改变，请以正式版上线后的合约规则为准。目前正处于UBS 2.0开发的第1阶段，按照第1阶段的内容及目标，我们已完成测试并正式上线了“UBS基金会”SERO权重功能，在接下来的第2~4阶段中，将逐步测试及上线UBSC权重、治理、分红等功能，预计将于第4阶段上线UBS基金会正式版。
`,
